...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        ...
        <div class="flex flex-col gap-4 sm:flex-row">
            ...
            <div class="filter-item" style="flex:1 1 160px; min-width:120px;">
                ...
                <div x-data="{ isOptionSelected: false }" class="relative z-20 bg-transparent">
                    <select name="tipe"
                        ...
                        <option value="" class="text-gray-700 dark:bg-gray-900 dark:text-gray-400">Semua Tipe</option>
                        <option value="pabrik" {{ ($tipe ?? '') == 'pabrik' ? 'selected' : '' }}>Pabrik</option>
                        <option value="retur" {{ ($tipe ?? '') == 'retur' ? 'selected' : '' }}>Retur</option>
                    </select>
            ...
            <div class="filter-btn-wrap" style="flex-shrink:0; padding-top:28px;">
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('laporan.barangmasuk.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
                ...
                @if(!empty($data))
                    ...
                    <tr>
                        ...
                        <td class="px-5 py-4 text-center">
                            <a href="{{ route('transaksi.barangmasuk.detail', $d->id) }}" class="text-sm font-semibold text-brand-500 hover:text-brand-600 dark:text-brand-400">{{ $d->no_surat ?? '-' }}</a>
                        </td>
                        <td class="px-5 py-4 text-sm text-center text-gray-500 dark:text-gray-400">{{ $d->tanggal ? $d->tanggal->format('d/m/Y') : '-' }}</td>
                        <td class="px-5 py-4 text-center">
                            @if($d->tipe == 'pabrik')
                                <span class="inline-flex items-center rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-500 dark:bg-brand-500/15 dark:text-brand-400">Pabrik</span>
                            @else
                        ...
                        <td class="px-5 py-4 text-center">
                            <span class="inline-flex items-center rounded-full bg-success-50 px-2.5 py-1 text-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-500">{{ $d->details->count() }}</span>
                        </td>
                    </tr>
                    @endforeach
                @else
                    <tr>
                        <td colspan="5" class="px-5 py-8 text-center text-sm text-gray-500 dark:text-gray-400">Data tidak ditemukan</td>
                    </tr>
                @endif
            </tbody>
        </table>
    ...
    <div class="px-5 py-4 border-t border-gray-200 dark:border-gray-800">
        {{ $data->appends(request()->only(['keyword', 'tanggal_awal', 'tanggal_akhir', 'tipe']))->links('vendor.pagination.tailadmin') }}
    </div>
    @endif
</div>
