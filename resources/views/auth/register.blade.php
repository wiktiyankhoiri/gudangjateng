...
<div>
    ...
    <form method="post" action="{{ route('register') }}">
        ...
        <div class="space-y-5">
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400" for="nama">
                    Nama Lengkap
                </label>
                <input
                    type="text"
                    ...
                    value="{{ old('nama') }}"
                    required
                    class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 @error('nama') border-error-500 focus:border-error-500 focus:ring-error-500/10 @enderror"
                />
                @error('nama')
    ...
    <div class="mt-6 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">
            Sudah punya akun?
            <a href="{{ route('login') }}" class="text-brand-500 hover:text-brand-600 font-medium">Masuk di sini</a>
        </p>
    </div>
</div>
