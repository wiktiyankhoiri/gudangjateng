...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-800">
        <x-search-bar route="{{ route('masterdata.barang.index') }}" query="{{ $q ?? '' }}" placeholder="Cari barang..." />
...
<div x-data="{ open: false }" @open-import.window="open = true" x-show="open" class="fixed inset-0 z-99999 flex items-center justify-center bg-gray-900/50 p-4" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
    <div @click.outside="open = false" class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Impor Barang Excel</h3>
        ...
        <form action="{{ route('masterdata.barang.import') }}" method="post" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    <input type="file" name="file_excel" id="fileExcelBarang" accept=".xls,.xlsx" required class="hidden" onchange="document.getElementById('fileLabelBarang').textContent = this.files[0]?.name || 'Pilih file'">
                    <label for="fileExcelBarang" class="flex h-11 w-full cursor-pointer items-center gap-3 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-500 shadow-theme-xs transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800">
                        ...
                        <span id="fileLabelBarang" class="truncate">Belum ada file dipilih</span>
                    </label>
                </div>
            </div>
