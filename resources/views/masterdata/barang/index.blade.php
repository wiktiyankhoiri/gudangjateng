...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-800"
         ...
         x-init="$watch('q', (value, oldValue) => {
             ...
             window._searchTimer = setTimeout(() => {
                 loading = true;
                 fetch('{{ route('masterdata.barang.index') }}?q=' + encodeURIComponent(value), {
        ...
        <div class="flex items-center gap-2">
            @if(!empty($q))
            <a href="{{ route('masterdata.barang.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
    ...
    <div id="table-wrap">
        @include('masterdata.barang._table')
    </div>
</div>
