    ...
    <table class="min-w-full">
        <thead>
            <tr class="border-b border-gray-100 dark:border-gray-800">
                <th class="px-5 py-3 text-center text-sm font-medium text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap">NO</th>
                <th class="px-5 py-3 text-center text-sm font-medium text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap">KODE</th>
                <th class="px-5 py-3 text-left text-sm font-medium text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap">NAMA BARANG</th>
                <th class="px-5 py-3 text-center text-sm font-medium text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap">SATUAN</th>
                <th class="px-5 py-3 text-center text-sm font-medium text-warning-600 uppercase dark:text-warning-400 whitespace-nowrap">HARGA GOLD</th>
                @if($showGrosir)
                <th class="px-5 py-3 text-center text-sm font-medium text-success-600 uppercase dark:text-success-400 whitespace-nowrap">HARGA GROSIR</th>
                @endif
                @if($showKhusus)
                <th class="px-5 py-3 text-center text-sm font-medium text-purple-600 uppercase dark:text-purple-400 whitespace-nowrap">HARGA KHUSUS</th>
                @endif
                @can('admin')
                <th class="px-5 py-3 text-center text-sm font-medium text-gray-500 uppercase dark:text-gray-400 whitespace-nowrap">AKSI</th>
                @endcan
            </tr>
        </thead>
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            <tr class="whitespace-nowrap">
                <td class="px-5 py-4 text-sm text-center text-gray-800 dark:text-white/90">{{ $b->id }}</td>
                <td class="px-5 py-4 text-center">
                    <span class="inline-flex items-center rounded-full bg-brand-50 px-2.5 py-1 text-sm font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
                        {{ esc($b->kode_barang) }}
                    </span>
                </td>
                <td class="px-5 py-4 text-sm text-left text-gray-800 dark:text-white/90">{{ esc($b->nama_barang) }}</td>
                <td class="px-5 py-4 text-center">
                    <span class="inline-flex items-center rounded-full bg-brand-50 px-2.5 py-1 text-sm font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
                        {{ strtoupper($b->satuan) }}
                    </span>
                ...
                <td class="px-5 py-4 text-center">
                    <div class="flex items-center justify-center gap-2">
                        <a href="{{ route('masterdata.barang.edit', $b->id) }}" class="inline-flex items-center justify-center rounded-lg bg-warning-50 p-2 text-warning-600 hover:bg-warning-100 dark:bg-warning-500/15 dark:text-warning-400">
                        ...
                        <form method="post" action="{{ route('masterdata.barang.delete', $b->id) }}" class="inline" data-confirm-message="Yakin ingin menghapus data barang ini?" data-confirm-ok="Ya, Hapus">
                            @csrf
...
<div class="border-t border-gray-200 dark:border-gray-800">
    {{ $barang->links('vendor.pagination.tailadmin') }}
</div>
@endif
