...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-800"
         x-data="{ q: '{{ $q ?? '' }}', loading: false }"
         x-init="$watch('q', (value, oldValue) => {
             if (value === oldValue) return;
             clearTimeout(window._searchTimer);
             window._searchTimer = setTimeout(() => {
                 loading = true;
                 fetch('{{ route('masterdata.toko.index') }}?q=' + encodeURIComponent(value), {
        ...
        <div class="flex items-center gap-2">
            @if(!empty($q))
            <a href="{{ route('masterdata.toko.index') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
            ...
            @endif
            <x-excel-dropdown
                template-route="{{ route('masterdata.toko.template') }}"
                export-route="{{ route('masterdata.toko.export') }}"
                import-route="#"
            />
            <a href="{{ route('masterdata.toko.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">
                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div id="table-wrap">
        @include('masterdata.toko._table')
    </div>
</div>
...
<div x-data="{ open: false }" @open-import.window="open = true" x-show="open" class="fixed inset-0 z-99999 flex items-center justify-center bg-gray-900/50 p-4" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
    <div @click.outside="open = false" class="w-full max-w-md rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Impor Toko Excel</h3>
        ...
        <form action="{{ route('masterdata.toko.import') }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="mb-4 rounded-lg bg-brand-50 p-4 dark:bg-brand-500/15">
                ...
                <p class="text-xs text-brand-600 dark:text-brand-400 mt-2 font-medium">💡 Jika kode_toko sudah ada, data akan diupdate.</p>
            </div>
            <div class="mb-4">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">File Excel</label>
                <div class="relative">
                    <input type="file" name="file_excel" id="fileExcelToko" accept=".xls,.xlsx" required class="hidden" onchange="document.getElementById('fileLabelToko').textContent = this.files[0]?.name || 'Pilih file'">
                    <label for="fileExcelToko" class="flex h-11 w-full cursor-pointer items-center gap-3 rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-500 shadow-theme-xs transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800">
                        ...
                        <span id="fileLabelToko" class="truncate">Belum ada file dipilih</span>
                    </label>
                </div>
            </div>
