...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    <div class="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-gray-800">
        <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">
            Detail Mutasi
            <span class="ml-2 text-sm font-normal text-gray-500">#{{ $mutasi->no_mutasi }}</span>
            @if($tipeMutasi && in_array($tipeMutasi, ['baik_ke_sales', 'sales_ke_baik']))
                <span class="ml-2 inline-flex items-center rounded-full bg-purple-50 px-2.5 py-1 text-xs font-medium text-purple-600 dark:bg-purple-500/15 dark:text-purple-400">Kanvas</span>
            @elseif($tipeMutasi)
                <span class="ml-2 inline-flex items-center rounded-full bg-warning-50 px-2.5 py-1 text-xs font-medium text-warning-600 dark:bg-warning-500/15 dark:text-warning-400">Kondisi</span>
            @endif
        </h5>
    </div>
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03] mt-4">
    <div class="px-5 py-4 border-b border-gray-200 dark:border-gray-800">
        <h5 class="text-lg font-semibold text-gray-800 dark:text-white/90">Detail Item Barang ({{ $totalItem }} item)</h5>
    </div>
        ...
        <table class="min-w-full table-sticky">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr>
                    ...
                    <td class="px-5 py-4 text-sm text-gray-800 dark:text-white/90">{{ $d->kode_barang ?? '-' }}</td>
                    <td class="px-5 py-4 text-sm text-gray-800 dark:text-white/90">{{ $d->nama_barang ?? '-' }}</td>
            ...
            <tfoot>
                <tr class="bg-gray-50 dark:bg-gray-800 font-semibold">
                    <td colspan="4" class="px-5 py-3 text-right text-sm text-gray-700 dark:text-gray-400">Total</td>
                    <td class="px-5 py-3 text-center text-sm text-gray-800 dark:text-white/90">{{ number_format($totalQty) }}</td>
                </tr>
            </tfoot>
            @endif
        </table>
