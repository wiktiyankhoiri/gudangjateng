...
<form data-protect-submit="true" data-confirm-message="Pastikan data sudah sesuai dengan nota fisik. Lanjutkan update?" data-confirm-ok="Ya, Update" method="post"
      action="{{ route('transaksi.barangmasuk.update', $data->id) }}">
...
foreach($detail as $d):
    ...
    if($stok && (int)$stok['stok_baik'] < (int)$d->qty_baik):
        ...
        $peringatanItems[] = sprintf('%s (jumlah masuk %d, stok baik saat ini %d, selisih -%d)', $nama, $d->qty_baik, $stok['stok_baik'], $selisih);
    endif;
endforeach;
@endphp
@if($adaPeringatan)
<div class="flex w-full border-l-6 border-warning-500 bg-warning-50 px-7 py-5 mb-6 dark:bg-warning-500/15 dark:border-warning-500/30" role="alert">
    ...
    <div class="w-full">
        ...
        <p class="text-sm text-gray-700 dark:text-white/80">Stok barang berikut sudah berkurang karena transaksi lain setelah barang masuk ini dibuat:</p>
...
<div class="grid grid-cols-1 gap-4 md:grid-cols-3">
    <div>
        ...
        <input type="text" name="no_surat" value="{{ old('no_surat', $data->no_surat) }}"
               class="h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 dark:focus:border-brand-800 uppercase" required
    ...
    <div>
        ...
        <div class="relative">
            <input type="text" name="tanggal" value="{{ old('tanggal', $data->tanggal ? $data->tanggal->format('Y-m-d') : date('Y-m-d')) }}"
    ...
    <div>
        ...
        <select name="tipe" id="tipe"
            ...
            <option value="pabrik" {{ old('tipe', $data->tipe) === 'pabrik' ? 'selected' : '' }}>Pabrik</option>
            <option value="retur" {{ old('tipe', $data->tipe) === 'retur' ? 'selected' : '' }}>Retur</option>
        </select>
    </div>
</div>

<!-- ROW 2: PABRIK / TOKO + KETERANGAN -->
<div class="grid grid-cols-1 gap-4 md:grid-cols-3 mt-4">
    <div>
        <div id="pabrikField">
            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">Pabrik</label>
            <select name="pabrik_id" class="h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 select2">
                <option value="">-- Pilih Pabrik --</option>
                @foreach($pabrik as $p)
                    <option value="{{ $p->id }}" {{ old('pabrik_id', $data->pabrik_id) == $p->id ? 'selected' : '' }}>{{ $p->kode_pabrik }} - {{ $p->nama_pabrik }}</option>
                @endforeach
            </select>
        </div>
        <div class="hidden" id="tokoField">
            ...
            <select name="toko_id" class="h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 select2">
                ...
                @foreach($toko as $t)
                    <option value="{{ $t->id }}" {{ old('toko_id', $data->toko_id) == $t->id ? 'selected' : '' }}>{{ $t->kode_toko }} - {{ $t->nama_toko }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<div class="flex items-center justify-between mb-4">
    ...
    <button type="button" id="btnTambah" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">
...
<div id="bodyBarang" class="space-y-2">
    ...
    <div class="flex flex-col sm:flex-row gap-2 items-start p-3 border border-gray-200 rounded-lg dark:border-gray-800 row-barang">
        ...
        <div class="w-full sm:w-24">
            ...
            <input type="number" name="qty_baik[]" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" min="0" value="{{ $d->qty_baik }}" required>
        </div>
        <div class="w-full sm:w-24 qty-rusak-field">
            <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">Jml Rusak</label>
            <input type="number" name="qty_rusak[]" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" min="0" value="{{ $d->qty_rusak }}">
        </div>
...
<div class="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-gray-800">
    <a href="{{ route('transaksi.barangmasuk.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function toggleHeader() {
        ...
        document.getElementById('pabrikField').classList.toggle('hidden', tipe !== 'pabrik');
        document.getElementById('tokoField').classList.toggle('hidden', tipe !== 'retur');
    }
    document.getElementById('tipe').addEventListener('change', toggleHeader);
    ...
    document.getElementById('btnTambah').addEventListener('click', function() {
        ...
        <div class="flex flex-col sm:flex-row gap-2 items-start p-3 border border-gray-200 rounded-lg dark:border-gray-800 row-barang">
            ...
            <div class="w-full sm:w-24">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">Jml Baik</label>
                <input type="number" name="qty_baik[]" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" min="0" value="1" required>
            </div>
            <div class="w-full sm:w-24 qty-rusak-field">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">Jml Rusak</label>
                <input type="number" name="qty_rusak[]" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" min="0" value="0">
            </div>
            <div class="w-full sm:w-20 flex justify-center" style="padding-top:28px;flex-shrink:0;">
                <button type="button" class="btnHapus inline-flex items-center justify-center gap-2 rounded-lg bg-error-50 p-2 text-error-600 hover:bg-error-100 dark:bg-error-500/15 dark:text-error-400">
                    <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M7.5 2.5C7.5 2.08579 7.83579 1.75 8.25 1.75H11.75C12.1642 1.75 12.5 2.08579 12.5 2.5V3.5H6.25V2.5H7.5ZM4.75 4.5V3.5C4.75 2.25736 5.75736 1.25 7 1.25H13C14.2426 1.25 15.25 2.25736 15.25 3.5V4.5H16.5C17.1904 4.5 17.75 5.05964 17.75 5.75C17.75 6.44036 17.1904 7 16.5 7H16.1273L15.2865 16.2885C15.1399 17.8235 13.8455 19 12.3043 19H7.6957C6.15448 19 4.86011 17.8235 4.71346 16.2885L3.87273 7H3.5C2.80964 7 2.25 6.44036 2.25 5.75C2.25 5.05964 2.80964 4.5 3.5 4.5H4.75ZM6.21817 7L7.04185 16.1443C7.07708 16.5095 7.39145 16.75 7.6957 16.75H12.3043C12.6086 16.75 12.9229 16.5095 12.9582 16.1443L13.7818 7H6.21817Z" fill="currentColor"/></svg>
