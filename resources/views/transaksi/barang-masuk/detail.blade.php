    ...
    <div class="p-5">
        <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
            <div>
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $barangMasuk->no_surat ?? '-' }}</p>
            </div>
            <div>
                <p class="mb-1 text-xs font-medium text-gray-500 uppercase dark:text-gray-400">Tanggal</p>
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $barangMasuk->tanggal ? $barangMasuk->tanggal->format('d-m-Y') : '-' }}</p>
            </div>
            <div>
                <p class="mb-1 text-xs font-medium text-gray-500 uppercase dark:text-gray-400">Tipe</p>
                <div>
                    @if($barangMasuk->tipe == 'pabrik')
                    ...
                    @else
                        <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-600 dark:bg-amber-500/15 dark:text-amber-400">Retur</span>
                    @endif
                </div>
            </div>
            <div>
                <p class="mb-1 text-xs font-medium text-gray-500 uppercase dark:text-gray-400">Pabrik/Toko</p>
                @php
                    $relasi = null;
                    if ($barangMasuk->tipe == 'pabrik' && $barangMasuk->pabrik) {
                        $relasi = $barangMasuk->pabrik->nama_pabrik;
                    } elseif ($barangMasuk->tipe == 'retur' && $barangMasuk->toko) {
                        $relasi = $barangMasuk->toko->nama_toko;
                    }
                @endphp
                @if($relasi)
                    @if($barangMasuk->tipe == 'pabrik')
                        <span class="inline-flex items-center rounded-full bg-brand-50 px-2.5 py-1 text-xs font-medium text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">{{ $relasi }}</span>
                    @else
                        <span class="inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-xs font-medium text-amber-600 dark:bg-amber-500/15 dark:text-amber-400">{{ $relasi }}</span>
                    @endif
                @else
            ...
            <div class="md:col-span-3">
                ...
                <p class="text-sm font-medium text-gray-800 dark:text-white/90">{{ $barangMasuk->keterangan ?: '-' }}</p>
            </div>
        </div>
    </div>
</div>
        ...
        <table class="min-w-full table-sticky">
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr>
                    ...
                    <td class="px-5 py-4 text-center text-sm font-semibold text-red-600 dark:text-red-400">{{ number_format($d->qty_rusak ?? 0) }}</td>
                </tr>
                @empty
                <tr>
