...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <form data-protect-submit="true" data-confirm-message="Pastikan data sudah sesuai dengan nota fisik. Lanjutkan simpan?" data-confirm-ok="Ya, Simpan" method="post"
          action="{{ route('transaksi.barangmasuk.store') }}">
        ...
        <div class="p-5 sm:p-6">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                ...
                <div>
                    ...
                    <select name="tipe" id="tipe"
                        ...
                        <option value="pabrik" {{ old('tipe') === 'pabrik' ? 'selected' : '' }}>Pabrik</option>
                        <option value="retur" {{ old('tipe') === 'retur' ? 'selected' : '' }}>Retur</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3 mt-4">
                <div>
                    <div id="pabrikTokoField">
                        <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400" id="pabrikTokoLabel">Pilih Pabrik / Toko</label>
                        <select name="pabrik_id" id="pabrikSelect" class="hidden h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 select2">
                            <option value="">-- Pilih Pabrik --</option>
                            @foreach($pabrik as $p)
                                <option value="{{ $p->id }}">{{ $p->kode_pabrik }} - {{ $p->nama_pabrik }}</option>
                            @endforeach
                        </select>
                        <select name="toko_id" id="tokoSelect" class="hidden h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 select2">
                            <option value="">-- Pilih Toko --</option>
                            @foreach($toko as $t)
                                <option value="{{ $t->id }}">{{ $t->kode_toko }} - {{ $t->nama_toko }}</option>
                            @endforeach
                        </select>
                        <input type="text" id="placeholderText" class="h-11 w-full rounded-lg border border-gray-300 dark:border-gray-700 dark:bg-dark-900 bg-transparent px-4 py-2.5 text-sm text-gray-500 dark:text-gray-400 shadow-theme-xs cursor-not-allowed focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:focus:border-brand-800" placeholder="Pilih Pabrik / Toko" readonly tabindex="-1">
                    </div>
                </div>
            ...
            <div class="flex flex-col sm:flex-row gap-2 items-end mb-6">
                ...
                <div class="flex gap-2 w-full sm:w-auto">
                    <div class="flex-1 sm:w-24">
                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 text-center">Jml Baik</label>
                        <input type="number" id="inputBaik" value="0" min="0"
                               class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-center text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    </div>
                    <div class="flex-1 sm:w-24">
                            <label class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 text-center">Jml Rusak</label>
                        <input type="number" id="inputRusak" value="0" min="0"
                               class="h-10 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-center text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                    </div>
                </div>
            ...
            <!-- DAFTAR BARANG (tabel) -->
            <div class="overflow-x-auto" id="listWrapper" style="display:none;">
                <table class="table-sticky min-w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            ...
                            <th class="px-5 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-gray-400 w-24 whitespace-nowrap">Jml Baik</th>
                            <th class="px-5 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-gray-400 w-24 whitespace-nowrap">Jml Rusak</th>
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-gray-800">
            <a href="{{ route('transaksi.barangmasuk.index') }}"
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var inputBaik = document.getElementById('inputBaik');
    var inputRusak = document.getElementById('inputRusak');

    // Init TomSelect untuk input barang
    ...
    function toggleHeader() {
        ...
        var pabrikSelect = document.getElementById('pabrikSelect');
        var tokoSelect = document.getElementById('tokoSelect');
        var placeholder = document.getElementById('placeholderText');
        var label = document.getElementById('pabrikTokoLabel');

        if (pabrikSelect.tomselect) pabrikSelect.tomselect.destroy();
        if (tokoSelect.tomselect) tokoSelect.tomselect.destroy();

        pabrikSelect.classList.add('hidden');
        tokoSelect.classList.add('hidden');
        placeholder.classList.remove('hidden');

        // Toggle Rusak input
        var isPabrik = (tipe === 'pabrik');
        inputRusak.disabled = isPabrik;
        ...
        if (tipe === 'pabrik') {
            pabrikSelect.classList.remove('hidden');
            placeholder.classList.add('hidden');
            label.textContent = 'Pabrik';
            new TomSelect(pabrikSelect, { maxItems:1, searchField:['text','value'], dropdownParent:'body', plugins:{clear_button:{title:'Hapus pilihan'}} });
        } else if (tipe === 'retur') {
            tokoSelect.classList.remove('hidden');
            placeholder.classList.add('hidden');
            label.textContent = 'Toko';
            new TomSelect(tokoSelect, { maxItems:1, searchField:['text','value'], dropdownParent:'body', plugins:{clear_button:{title:'Hapus pilihan'}} });
        } else {
            label.textContent = 'Pilih Pabrik / Toko';
        }
    }
    ...
    document.getElementById('btnTambah').addEventListener('click', function() {
        var barangId = inputBarang.value;
        ...
        var qtyBaik = parseInt(inputBaik.value) || 0;
        var qtyRusak = parseInt(inputRusak.value) || 0;

        if (qtyBaik + qtyRusak < 1) {
            showSystemMessage('warning', 'Minimal total jumlah baik + rusak adalah 1.');
            return;
        }
    ...
    // Enter key di input quantity langsung tambah
    ...
    inputBaik.addEventListener('keydown', function(e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            document.getElementById('btnTambah').click();
        }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (daftarBarang.querySelectorAll('tr').length === 0) {
