...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <form data-protect-submit="true" data-confirm-message="Pastikan data sudah sesuai dengan nota fisik. Lanjutkan update?" data-confirm-ok="Ya, Update" method="post"
          action="{{ route('transaksi.barangkeluar.update', $data->id) }}">
        ...
        <div class="flex w-full border-l-6 border-warning-500 bg-warning-50 px-7 py-5 mb-6 dark:bg-warning-500/15 dark:border-warning-500/30" role="alert">
            ...
            <div class="w-full">
                ...
                <p class="text-sm text-gray-700 dark:text-white/80">Stok barang berikut sudah berkurang karena transaksi lain setelah barang keluar ini dibuat:</p>
                ...
                <div>
                    ...
                    <select name="sales_id" class="h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 select2">
                        ...
                        @foreach($salesList as $s)
                            <option value="{{ $s->id }}" {{ old('sales_id', $data->sales_id) == $s->id ? 'selected' : '' }}>{{ $s->nama }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="flex flex-col sm:flex-row gap-2 items-start p-3 border border-gray-200 rounded-lg dark:border-gray-800 row-barang">
                    <div class="w-full" style="flex:1 1 0%; min-width:200px;">
                        ...
                        <select name="barang_id[]" class="h-11 w-full rounded-lg border dark:bg-dark-900 border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 shadow-theme-xs focus:border-brand-300 focus:outline-hidden focus:ring-3 focus:ring-brand-500/10 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:focus:border-brand-800 barangSelect" required>
                            ...
                            @foreach($barang as $b)
                                <option value="{{ $b->id }}" {{ old('barang_id.'.$loop->index, $d->barang_id) == $b->id ? 'selected' : '' }}>{{ $b->kode_barang }} - {{ $b->nama_barang }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-full sm:w-24">
                        ...
                        <input type="number" name="qty_baik[]" class="dark:bg-dark-900 shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm text-gray-800 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90" min="0" value="{{ old('qty_baik.'.$loop->index, $d->qty_baik) }}" required>
                    </div>
        ...
        <div class="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-gray-800">
            <a href="{{ route('transaksi.barangkeluar.index') }}" class="inline-flex items-center gap-2 rounded-lg bg-white px-4 py-3 text-sm font-medium text-gray-700 shadow-theme-xs ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700 dark:hover:bg-white/[0.03]">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('btnTambah').addEventListener('click', function() {
        ...
        <div class="flex flex-col sm:flex-row gap-2 items-start p-3 border border-gray-200 rounded-lg dark:border-gray-800 row-barang">
            <div class="w-full" style="flex:1 1 0%; min-width:200px;">
                ...
                <p class="stok-info mt-1 text-xs hidden"></p>
            </div>
            <div class="w-full sm:w-24">
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-400 mb-1.5">Jml Baik</label>
    ...
    document.addEventListener('click', function(e){
        ...
        if (!btn) return;
        if (document.querySelectorAll('#bodyBarang > div').length > 1) { btn.closest('.row-barang').remove(); }
        else { showSystemMessage('warning', 'Minimal harus ada 1 barang.'); }
    });

    // STOK INFO
    var stokData = [];
    try { stokData = @json($stokPerBarang ?? []); } catch(e) {}
    var stokIndex = {};
    ...
    document.addEventListener('change', function(e) {
        if (e.target.classList.contains('barangSelect')) {
            ...
            if (stok) {
                info.innerHTML = '<span class="font-medium text-warning-500 dark:text-warning-400">Baik:</span> ' + stok.stok_baik + ' <span class="font-medium text-warning-500 dark:text-warning-400">Rusak:</span> ' + stok.stok_rusak;
                info.classList.remove('hidden');
            } else {
                info.innerHTML = '<span class="text-warning-500 dark:text-warning-400">Belum ada stok</span>';
                info.classList.remove('hidden');
            }
        }
    });
