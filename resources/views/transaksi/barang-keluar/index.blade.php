    ...
    <div class="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between border-b border-gray-200 dark:border-gray-800">
        <form method="get" class="flex items-center gap-2">
            <div class="relative">
                ...
                <input type="text" name="cari" value="{{ $cari ?? '' }}" class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full sm:w-[280px] rounded-lg border border-gray-300 bg-transparent bg-none px-4 py-2.5 pl-[62px] text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" placeholder="Cari no surat, toko, sales...">
            </div>
        ...
        <a href="{{ route('transaksi.barangkeluar.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-3 text-sm font-medium text-white shadow-theme-xs transition hover:bg-brand-600">
            <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="overflow-x-auto"><table class="table-sticky min-w-full">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-800">
                    ...
                    <th class="px-5 py-3 text-center text-xs font-medium text-gray-500 uppercase dark:text-gray-400">SUMBER</th>
            ...
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                ...
                <tr>
                    ...
                    <td class="px-5 py-4 text-sm text-center text-gray-800 dark:text-white/90">{{ esc($d->nama_toko) }}</td>
                    <td class="px-5 py-4 text-sm text-center text-gray-500 dark:text-gray-400">{{ esc($d->nama_sales) }}</td>
                    ...
                    <td class="px-5 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('transaksi.barangkeluar.detail', $d->id) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-brand-50 p-2 text-brand-600 hover:bg-brand-100 dark:bg-brand-500/15 dark:text-brand-400" title="Lihat Detail">
                                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <a href="{{ route('transaksi.barangkeluar.edit', $d->id) }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-warning-50 p-2 text-warning-600 hover:bg-warning-100 dark:bg-warning-500/15 dark:text-warning-400">
                                <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <form method="post" action="{{ route('transaksi.barangkeluar.delete', $d->id) }}" class="inline" data-confirm-message="Yakin ingin menghapus data barang keluar ini?" data-confirm-ok="Ya, Hapus">
                                @csrf
                ...
                <tr>
                    <td colspan="8" class="px-5 py-12 text-center">
                        <svg class="mx-auto mb-4 h-16 w-16 text-gray-300 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 11.25l-3-3m0 0l-3 3m3-3v7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        </svg>
                        <p class="text-sm text-gray-500 dark:text-gray-400">Belum ada data barang keluar</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
